...
// ─── PTL Chat — TODO: nuevo enfoque ──────────────────────────────────────────
// Los helpers confluenceRequest / searchConfluence / getConfluencePage
// y los endpoints /api/ptl/epics + /api/ptl/epic/:epicKey quedan activos.
// El chat con LLM se reimplementará con fuentes: Jira + Confluence + Drive.

app.get('/api/ptl/status', (req, res) => {
  res.json({ provider: null, model: null, status: 'coming_soon' });
});

app.post('/api/ptl/chat', (req, res) => {
  res.status(503).json({ error: 'PTL Chat en rediseño. Próximamente.' });
});
